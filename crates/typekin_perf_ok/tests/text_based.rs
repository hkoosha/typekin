...
#[test]
fn text_based_optimized_const_integral_operations_match_raw_assembly() {
...
fn assert_operations_match(binary: &Path) {
    ...
    if !diffs.is_empty() {
        panic!(
            "optimized const integral wrapper operations produced different text assembly:\n\n{}",
            diffs.join("\n")
        );
    }
}
